...
class TOTPManagerPro:
    def __init__(self, root):
        self.root = root
        self.root.title("TOTP 验证码管理器 Pro")
        ...
        self.accounts = []
        self.groups = {}  # 分组字典
        self.filtered_accounts = []  # 过滤后的账户
        self.labels = []
        
        # 界面状态
        self.items_per_page = 10
        self.current_page = 0
        self.current_group = "全部"  # 当前选中的分组
        self.search_query = ""
        ...
        for idx, acc in enumerate(sub_accounts):
            ...
            if secret_norm:
                try:
                    ...
                    code_label.config(text="密钥错误", fg="#e74c3c")
    ...
    def update_page_buttons(self):
        ...
        self.prev_btn.config(
            ...
            bg="#3498db" if self.current_page > 0 else "#95a5a6"
        )
        self.next_btn.config(
            state="normal" if self.current_page < total_pages - 1 else "disabled",
            bg="#3498db" if self.current_page < total_pages - 1 else "#95a5a6"
        )
        ...
        for code_label, time_label, totp, tip_label in self.labels:
            if totp is None:
                ...
                try:
                    ...
                    code_label.config(text=code, fg="#3498db")
                    time_label.config(
                        text=f"{remain}s",
                        fg="#e74c3c" if remain <= 5 else "#f39c12" if remain <= 10 else "#27ae60"
                    )
                except Exception:
                    code_label.config(text="生成失败", fg="#e74c3c")
                    time_label.config(text="--s")
